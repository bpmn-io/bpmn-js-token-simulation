  ...
  const behaviors = {
    ...
    'bpmn:ErrorEventDefinition': (context) => {
      ...
      do {
        ...
        if (!isEventBasedSubProcess(parentScope.element)) {
          break;
        }
...
function isEventBasedSubProcess(element) {
  return is(element, 'bpmn:SubProcess') && element.businessObject.triggeredByEvent;
}