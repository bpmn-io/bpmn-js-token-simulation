...
EventBehaviors.prototype.get = function(element) {
  ...
  const behaviors = {
    ...
    'bpmn:CancelEventDefinition': (context) => {
      ...
      const transactionScope = this._transactionBehavior.findTransactionScope(scope);
    ...
    'bpmn:CompensateEventDefinition': (context) => {
      ...
      // find event subscription in scope parent's children
      const subscriptions = this._simulator.findSubscriptions({
        event: element
      });

      const subscription = subscriptions.find(subscription => sameParents(subscription.scope, scope));

      if (subscription) {
        return this._simulator.waitForScopes(
          scope,
          this._simulator.trigger(subscription)
        );
      }
    }
  };
...
function sameParents(scopeA, scopeB) {

  const parents = new Set();

  while (scopeA || scopeB) {

    if (parents.has(scopeA)) {
      return true;
    }
