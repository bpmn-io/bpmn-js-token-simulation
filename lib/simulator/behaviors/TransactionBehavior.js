...
TransactionBehavior.prototype.makeCompensable = function(scope) {
  ...
  if (scope.hasTrait(ScopeTraits.COMPENSABLE)) {
    return;
  }
  ...
  console.log('makeCompensable ', { scope });

  this._simulator.subscribe(scope, compensateEvent, (initiator) => {
...
TransactionBehavior.prototype.compensate = function(context) {
  ...
  console.log('compensate', { context, localSubscriptions, otherSubscriptions });

  const trigger = scope.active ? (subscription) => {
