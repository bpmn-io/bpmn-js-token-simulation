...
export default function Simulator(injector, eventBus, elementRegistry) {
  ...
  function trigger(context) {
    ...
    const subscriptions = Array.from(scope.subscriptions);

    const event = getEvent(_event);

    const matchingSubscriptions = subscriptions.filter(
      subscription => eventsMatch(event, subscription.event)
    );
    ...
    const interrupt = matchingSubscriptions.find(
      subscription => subscription.event.interrupting
    );
    ...
    const remainingSubscriptions = matchingSubscriptions.filter(
      subscription => isRethrow(subscription.event, interrupt.event)
    );
    ...
    return [ interrupt.triggerFn(initiator) ];
  }
  ...
  function getEvent(element) {
    ...
    if (is(eventDefinition, 'bpmn:CompensateEventDefinition')) {
      return {
        element,
        type: 'compensate',
        ref: eventDefinition.activityRef,
        interrupting,
        boundary
      };
    }
  ...
  function findSubscriptions(filter) {
    ...
    const subscribedTo = subscriptionFilter(filter);

    return findScopes({
      subscribedTo,
      trait: ScopeTraits.NOT_DEAD
    }).map(
      scope => Array.from(scope.subscriptions).filter(subscribedTo)
    ).flat();
  }
  ...
  function findSubscription(filter) {
    ...
    return scope && Array.from(scope.subscriptions).filter(subscribedTo)[0];
  }
  ...
  function initializeScope(context) {
    ...
    for (const childElement of children) {
      ...
      if (isEventSubProcess(childElement)) {
        const startEvents = childElement.children.filter(isStartEvent);

        for (const startEvent of startEvents) {
          subscribe(scope, startEvent, initiator => signal({
    ...
    for (const attacher of attachers) {
      ...
      if (isBoundaryEvent(attacher) && !isCompensationEvent(attacher)) {
        ...
        subscribe(scope, attacher, initiator => signal({
          element: attacher,
