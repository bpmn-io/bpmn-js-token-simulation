...
export default function ShowScopes(
  ...
  eventBus.on(SCOPE_FILTER_CHANGED_EVENT, event => {
    ...
    for (const element of allElements) {
      ...
      domClasses(element).toggle('focussed', this._scopeFilter.isFocused(scopeId));
...
ShowScopes.prototype._init = function() {
  ...
  // Insert before the overlays container so scope instance buttons
  // appear before context pad buttons in the tab sequence.
  this._canvas.getContainer().insertBefore(
    this._container,
...
ShowScopes.prototype.addScope = function(scope) {
  ...
  domClasses(html).toggle('focussed', this._scopeFilter.isFocused(scope));
