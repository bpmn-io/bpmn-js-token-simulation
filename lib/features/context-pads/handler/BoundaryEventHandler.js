...
BoundaryEventHandler.prototype.createContextPads = function (element) {
  if (element.attachers.length === 0) {
    console.log('no attachers');
    return;
  }

  if (this._processInstances
  ...
  element.attachers.forEach(function (attachedElement) {
    ...
    domEvent.bind(contextPad, 'click', function () {
      self._processInstances
        ...
        var newProcessInstanceId = processInstance.parentProcessInstanceId;
        if(attachedElement.businessObject.cancelActivity) {
          // This is an interrupting BoundaryEvent
          ...
          self._eventBus.fire(PROCESS_INSTANCE_FINISHED_EVENT, processInstance);
          self._eventBus.fire(UPDATE_ELEMENT_EVENT, {
            element: element
          });
        } else {
          // This is an non-interrupting BoundaryEvent
          newProcessInstanceId = self._processInstances.create(attachedElement, processInstance.parentProcessInstanceId);
        }
