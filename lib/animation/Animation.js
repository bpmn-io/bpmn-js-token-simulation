...
Animation.prototype.createAnimation = function(connection, scope, done=noop) {
  ...
  const tokenGfx = this._createTokenGfx(scope.color || TOKEN_COLOR);
  ...
  if (this._hiddenScopes.has(scope)) {
    animation.hide();
  }
...
Animation.prototype._createTokenGfx = function(color) {
...
Animation.prototype.showScope = function(scope) {
  ...
  this._hiddenScopes.delete(scope);
};

Animation.prototype.hideScope = function(scope) {
  this._animations.forEach(function(animation) {
    if (animation.scope === scope) {
      animation.hide();
    }
