...
function Animation(canvas, eventBus) {
  ...
  eventBus.on(PROCESS_INSTANCE_FINISHED_EVENT, function(context) {
    self.animations.forEach(function (animation) {
      if (animation.processInstanceId == context.processInstanceId
      || animation.processInstanceId == context.parentProcessInstanceId) {
        animation.animation.stop();
    ...
    // TODO: Check if new code above is acceptable for all cases
