...
Animation.prototype.toggleCandle = function() {
...
Animation.prototype._createTokenGfx = function(processInstanceId) {
  var parent = this.group
    .group()
    .attr('class', this.candle ? 'token candle' : 'token')
    .hide();

  parent
    .circle(TOKEN_SIZE, TOKEN_SIZE)
    // .attr('fill', '#52b415')
    .attr('fill', this.candle ? 'none' : '#52b415')
    .attr('class', 'circle');

  parent
    // .text(processInstanceId.toString())
    .text(this.candle ? '🕯️' : processInstanceId.toString())
